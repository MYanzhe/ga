...
+(NSString*) getOtherString{
    NSString *deviceName = [DeviceInfoUtil getDeviceName];
    ...
    NSString *otherStr = [NSString stringWithFormat:@"设备型号-->%@, iPhone名称-->%@, app版本号-->%@, 电池电量-->%f, 当前系统名称-->%@,平台类型-->IOS平台",deviceName,iPhoneName,appVerion,batteryLevel,systemName];
    return otherStr;
}
...
+ (void)getDeviceInfo
{
    ...
    // 这个方法后面会单独列出
    NSString *macAddress = [DeviceInfoUtil getMacAddress];
    NSLog(@"macAddress-->%@", macAddress);
    
    // 这个方法后面会单独列出
    NSString *deviceIP = [DeviceInfoUtil getDeviceIPAddresses];
...
//    NSString *phonenum = [self getPhoneNum];
    NSString *uuid = [DeviceInfoUtil getUUID];

    NSString *macAddress = [DeviceInfoUtil getMacAddress];
    NSString *otherDetile = [DeviceInfoUtil getOtherString];

//    [appInfoDict setObject:phonenum forKey:@"phonenum"];
    [appInfoDict setObject:uuid forKey:@"uuid"];
    [appInfoDict setObject:macAddress forKey:@"macAddress"];
    [appInfoDict setObject:otherDetile forKey:@"phoneDetile"];
    
    NSLog(@"getAppInfo:%@",appInfoDict);
    return [appInfoDict copy];
}
